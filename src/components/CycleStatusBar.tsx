...
export const CycleStatusBar = React.memo(function CycleStatusBar({
  ...
  /** Valide le CLEAR READ en INSCRIVANT les VGI's (oui/non). */
  ...
  // Le cycle NULL a sa PROPRE fin (CLEAR READ validé avec les VGI's) → pas de bouton AS-IS ici.
  ...
  return (
    <div style={{ display: 'flex', gap: 8, alignItems: 'center', justifyContent: 'flex-start', flexWrap: 'wrap', width: '100%', marginTop: 6, pointerEvents: 'auto' }}>
      ...
      {/* ── CYCLE NULL ────────────────────────────────────────────────────────────
          ...
          l'écart de TA depuis le départ du null + la validation du CLEAR READ avec les VGI's. */}
      {isNull && (
        <div style={{ ...chip, background: isLightTheme ? 'rgba(34,211,238,0.08)' : 'rgba(255,255,255,0.06)', border: `1px solid ${isLightTheme ? 'rgba(34,211,238,0.30)' : 'rgba(255,255,255,0.22)'}` }}>
      ...
      {isNull && nullPhase === 'clear_read' && (
        <div style={{ display: 'flex', gap: 6 }}>
          <button onClick={() => onValidateClearRead?.(true)} title={L("Valida il CLEAR READ — VGI's presenti",
                   "Valide le CLEAR READ — VGI's présents",
                   "Validate the CLEAR READ — VGI's present",
                   "Valida el CLEAR READ — VGI's presentes",
                   "Validera CLEAR READ — VGI's närvarande")}
            ...
            <span style={{ fontSize: 12, letterSpacing: '0.06em' }}>▸ CLEAR READ · VGIs</span>
          </button>
          <button onClick={() => onValidateClearRead?.(false)} title={L('Valida il CLEAR READ — nessun VGI',
                   'Valide le CLEAR READ — aucun VGI',
                   'Validate the CLEAR READ — no VGI',
                   'Valida el CLEAR READ — ningún VGI',
                   'Validera CLEAR READ — inga VGI')}
