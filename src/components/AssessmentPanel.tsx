...
export function AssessmentPanel({ items, onHide, t, readMeta }: AssessmentPanelProps) {
  ...
  return (
    <div
      className={`p-2 flex flex-col gap-1.5 ${collapsed ? '' : 'flex-1 min-h-0'}`}
      style={{
    ...
    >
      ...
      {!collapsed && (
        <div className="flex-1 overflow-y-auto flex flex-col gap-1 min-h-0">
          {items.length === 0 && (
            <span className="text-[10px] italic py-1" style={{ color: isLightTheme ? '#94a3b8' : 'rgba(255,255,255,0.4)' }}>
              {t('assessment_empty')}
            </span>
          )}
            ...
            return (
              ...
              >
                <span className="flex items-baseline gap-1.5 min-w-0">
                  <span className="text-[9px] shrink-0" style={{ color: isLightTheme ? '#94a3b8' : 'rgba(255,255,255,0.4)' }}>
                    [{a.time.toFixed(0)}s]
                  </span>
                  <span className="text-[11px] font-mono truncate" style={{ color: isLightTheme ? '#0f172a' : 'rgba(255,255,255,0.92)' }}>
                    {a.item}
                  </span>
                </span>
                <span className="text-[11px] font-mono font-bold shrink-0 flex items-baseline gap-1" style={{ color: m.color }}>
                  {m.short}
                  {a.reaction !== 'NULL' && a.reaction !== '⏳' && a.beforeMs ? (
                    <span className="text-[9px] font-normal" style={{ color: isLightTheme ? '#64748b' : 'rgba(255,255,255,0.5)' }}>−{a.beforeMs}ms</span>
                  ) : null}
                </span>
              </div>
            );
